...
function snapshot(db) {
  return {
    reset: Object.fromEntries(UNIVERSE_TABLES.map((name) => [name, countTable(db, name)])),
    preserved: Object.fromEntries(PRESERVED_TABLES.map((name) => [name, countTable(db, name)])),
  };
}

function resetUniverseWorkspace(db) {
  const before = snapshot(db);
  db.exec("PRAGMA foreign_keys = ON;");
  db.exec("BEGIN IMMEDIATE;");
  try {
    for (const table of UNIVERSE_TABLES) {
  ...
  const removed = Object.fromEntries(UNIVERSE_TABLES.map((name) => {
