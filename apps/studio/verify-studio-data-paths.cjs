...
assert.equal(
  resolveDataRoot({ projectRoot, environment: { CHANNEL_FOUNDRY_DATA_ROOT: "workspace-data" } }),
  path.join(projectRoot, "workspace-data"),
);
assert.equal(
  dataPaths({ projectRoot, environment: { CHANNEL_FOUNDRY_DATA_ROOT: "workspace-data" } }).studio,
  path.join(projectRoot, "workspace-data", "studio"),
);

const directory = fs.mkdtempSync(path.join(os.tmpdir(), "channel-foundry-data-paths-"));
try {
  ...
  const configured = configureElectronDataPaths(fakeApp, {
    projectRoot,
    environment: { CHANNEL_FOUNDRY_DATA_ROOT: configuredRoot },
  });
